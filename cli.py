...
parser.add_argument("-a", "--amount",  type=int, help="Necessary amount of money to withdrawal money")
...
parser.add_argument("-cht", "--check_telephone_pay", help="Check about mobile account replenishment")
parser.add_argument("-chw", "--check_withdrawal", help="Check about amount of withdrawal money")
...
if pin_validate(str(args.p), password, Controller(CardAccount()).from_json_information_about_card("status")) == False and pin_validate(str(args.sp), password, Controller(CardAccount()).from_json_information_about_card("status")) == False and pin_validate(str(args.tp), password, Controller(CardAccount()).from_json_information_about_card("status")) == False:
    ...
    if args.withdrawal:
        ...
        remainder_amount_in_storage_of_banknotes = Controller(StorageOfBanknotes()).from_json_storage_balance(str(args.nominal))
        if card_balance - args.withdrawal >= 0 and remainder_amount_in_storage_of_banknotes - args.amount >= 0:
            Controller(CardAccount()).withdrawal(card_balance, args.withdrawal, args.nominal, args.amount, summary_withdrawal_money, remainder_amount_in_storage_of_banknotes)
            shutil.copyfile("Utility/Check_pattern.txt", "D:\\Python\\ppvis4\\Utility\\Check_withdrawal.txt")
            with open("Check_withdrawal.txt", "a") as document:
                ...
                document.write("\nCASH WITHDRAWAL: " + str(Controller(CardAccount()).from_json_information_about_card("withdrawal")))
    ...
    if args.telephone_pay:
        if args.balance - args.telephone_pay >= 0:
            ...
            card_balance = Controller(CardAccount()).from_json_information_about_card("balance")
            final_card_balance = card_balance - args.telephone_pay
            Controller(CardAccount()).write_to_json_information_about_card("balance", final_card_balance)
            telephone_balance_after_payment = telephone_balance + args.telephone_pay
            ...
            shutil.copyfile("Utility/Check_mobile_pattern.txt", "D:\\Python\\ppvis4\\Utility\\Check_telephone_bill.txt")
